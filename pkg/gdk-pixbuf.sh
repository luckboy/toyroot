...
CC="$MUSL_GCC" CPPFLAGS="-I$ROOT_DIR/bin/$ARCH/jpeg_dev/usr/include" CFLAGS="$PKG_CFLAGS" LDFLAGS="$PKG_LDFLAGS" LIBS="$PKG_LIBS $PKG_TIFF_LIBS $PKGCFG_LIBPNG_LIBS -lffi" STRIP="$STRIP" \
BASE_DEPENDENCIES_CFLAGS="$PKGCFG_GLIB_CFLAGS $PKGCFG_TIFF_CFLAGS $PKGCFG_LIBPNG_CFLAGS -I$ROOT_DIR/bin/$ARCH/jpeg_dev/usr/include" \
BASE_DEPENDENCIES_LIBS="$PKGCFG_GLIB_LIBS $PKGCFG_TIFF_LIBS $PKGCFG_LIBPNG_LIBS -ljpeg" \
